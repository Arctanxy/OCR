...
class AudioSetDataset(object):
    def __init__(self, sample_rate=32000, data_dir = r'C:\scripts\鸟叫声识别数据\train_data\train_data\\', skip = False):
        ...
        for root, dirs, files in os.walk(data_dir):
            for file in files:
                audio_name = os.path.join(root, file)
                if audio_name.split(".")[-1] == "pk":
                    continue
        ...
        for index in tqdm(range(min(len(self.audio_names), 3000))):
            audio_name = self.audio_names[index]
            if index % 1000 == 0:
                print("load %d samples" % index)
            audio_name=self.audio_names[index]
            try:
                # waveform, _=librosa.load(audio_name, sr = self.sample_rate)
                # waveform, _=librosa.load(audio_name, sr = 32000)
                sr, waveform = read(audio_name)
                ...
                print(audio_name)
                import pdb;pdb.set_trace()
            label = int(audio_name[-12:-9])
            target = np.zeros(100)
            target = target.astype('float32')
            target[label] = 1
            data_dict = {
                'audio_name': audio_name, 'waveform': waveform, 'target': target}
            self.buffer_list.append(data_dict)
    ...
    def __getitem__(self, index):
        ...
        if index < 3000:
            ...
            return data_dict #waveform, target
