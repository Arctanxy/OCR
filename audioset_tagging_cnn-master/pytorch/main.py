...
def train(args):
    ...
    checkpoints_dir=r'C:\scripts\audioset_tagging_cnn-master\checkpoints'
    ...
    #     'augmentation={}'.format(augmentation), 'batch_size={}'.format(batch_size),
    #     'statistics.pkl')
    statistics_path = r'E:\workspace\classify\audioset_tagging_cnn-master\audioset_tagging_cnn-master\statistics_path\statistics.pkl'
    create_folder(r'E:\workspace\classify\audioset_tagging_cnn-master\audioset_tagging_cnn-master\statistics_path') #os.path.dirname
    ...
    dataset_test = AudioSetDataset(sample_rate=sample_rate, data_dir = r"C:\scripts\鸟叫声识别数据\dev_data\\", skip = True)
    ...
    eval_bal_loader = torch.utils.data.DataLoader(dataset=dataset_test,
        num_workers=num_workers, pin_memory=True, shuffle=True)

    eval_test_loader = torch.utils.data.DataLoader(dataset=dataset_test,
        num_workers=num_workers, pin_memory=True, shuffle=True)
    ...
    for batch_data_dict in train_loader:
        ...
        if (iteration % 2000 == 0 and iteration > resume_iteration) or (iteration == 0):
            ...
            logging.info('Validate test mAP: {:.3f}'.format(
                np.mean(test_statistics['average_recall'])))
        ...
        if 'mixup' in augmentation and 0:
...
if __name__ == '__main__':
    ...
    parser_train.add_argument('--batch_size', type=int, default=24)
